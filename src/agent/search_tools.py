from types_defs.state_type import SubQueryInfo
...
class VectorSearcher:
    def __init__(
        ...
        spec_collection: str = "supermicro_spec",
    ...
    async def group_and_slim(self, items):
        ...
        for item in items:
            ...
            key = (meta.get("product_model"), meta.get("doc_type"))
            if not key[0] or not key[1]:
                continue
            grouped[key].append(item)

        results = []
        for (product_model, doc_type), group_items in grouped.items():
    ...
    async def run_spec(self, sub_query: SubQueryInfo, top_k: Optional[int] = None) -> SubQueryInfo:
        ...
        product_models = sub_query.slots[0].product_model or []
        print("[slots.product_model] →", product_models)
        ...
        if product_models:
            ...
            print("No product_model, fallback only using doc_type filter")
        ...
        await self.rerank_spec_results(sub_query, top_k=top_k)
