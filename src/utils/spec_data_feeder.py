...
COLLECTION_NAME = "supermicro_spec_2"
...
The motherboard name is: {motherboard_name}
The motherboard family is: {motherboard_generation}
The system name is : {system_name}

The full specification data is shown below:
{data_row}

For each specification field in row, try to generate QA pairs to help the user understand the system or its features.
...
[
  {{
    ...
    "metadata": {{
      ...
      "product_family": "{motherboard_generation}",
      "system_model": "{system_name}", //If the system name is not available or empty, do not include the "system_model" field in the metadata.
      "product_model": "{motherboard_name}",
      "product_type": "server motherboard",
...
def generate_qa_from_row(row: dict, timeout_sec=60) -> list:
    motherboard_name = safe_get(row, "motherboard_name") or "Unknown Model"
    motherboard_family = safe_get(row, "motherboard_family") or "Unknown Family"
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        motherboard_family=motherboard_family,
